...
public class NotaSaidaController {
    ...
    @PutMapping("/{numero}/{modelo}/{serie}/{clienteId}/cancelar")
    @Operation(summary = "Cancela uma nota de saída e todas as suas parcelas")
    public ResponseEntity<?> cancelar(
            ...
            @PathVariable Long clienteId) {
        try {
            System.out.println("=== CONTROLLER CANCELAR NOTA SAÍDA ===");
            System.out.println("Chave: " + numero + "/" + modelo + "/" + serie + "/" + clienteId);
            
            notaSaidaService.cancelarNota(numero, modelo, serie, clienteId);
            ...
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ErrorResponse("Erro ao cancelar nota de saída"));
        }
    }
